/**
 * Mutatio MCP Hub — Cloudflare Remote MCP Server (stateless Streamable HTTP)
 *
 * Connect in Claude Code:
 *   claude mcp add --transport http mutatio-hub https://mutatio-mcp-hub.blewisorlando.workers.dev/mcp
 *
 * Tools:
 *  CODING:    get_token_stats, search_github_code, fetch_doc_snippet
 *  MARKETING: get_financial_signals, summarise_for_stakeholder
 *  MEMORY:    save_context, recall_context, list_saved_context
 */
...
function registerTools(server: McpServer, env: Env): void {
  ...
  server.tool(
    ...
    {},
    async () => {
      const r = await fetch(`${env.TOKEN_PROXY_URL}/stats`);
  ...
  server.tool(
    ...
    async ({ query, repo, language }) => {
      if (!env.GITHUB_TOKEN) {
        return { content: [{ type: 'text' as const, text: 'GITHUB_TOKEN secret not set. Run: wrangler secret put GITHUB_TOKEN in E:\\cf-mcp-hub' }] };
      }
  ...
  server.tool(
    ...
    async ({ ticker }) => {
      ...
      const cached = await env.HUB_CACHE.get(cacheKey);
      ...
      await env.HUB_CACHE.put(cacheKey, result, { expirationTtl: 300 });
  ...
  server.tool(
    ...
    async ({ content, audience }) => {
      ...
      const cached = await env.HUB_CACHE.get(`summary:${hash}`);
      if (cached) return { content: [{ type: 'text' as const, text: `[cached]\n${cached}` }] };
      const summary = `[${audience.toUpperCase()} SUMMARY REQUEST]\nPersona: ${personas[audience]}\n\nSource content:\n${content.slice(0, 3000)}`;
      await env.HUB_CACHE.put(`summary:${hash}`, summary, { expirationTtl: 3600 });
  ...
  server.tool(
    ...
    async ({ key, value, ttl_hours }) => {
      await env.HUB_CACHE.put(`ctx:${key}`, value, { expirationTtl: ttl_hours * 3600 });
  ...
  server.tool(
    ...
    { key: z.string() },
    async ({ key }) => {
      const val = await env.HUB_CACHE.get(`ctx:${key}`);
  ...
  server.tool(
    ...
    {},
    async () => {
      const list = await env.HUB_CACHE.list({ prefix: 'ctx:' });
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname !== '/mcp') {
      return new Response(JSON.stringify({ name: 'mutatio-mcp-hub', status: 'ok', endpoint: '/mcp' }), {
        headers: { 'Content-Type': 'application/json' },
      });
    }

    const server = new McpServer({ name: 'mutatio-mcp-hub', version: '1.0.0' });
